{{--   <!DOCTYPE html>
  ...
  <head>
      ...
      <style>
          ...
          .section {
              margin-bottom: 10px;
              text-align: center; /* Center the section contents */
          }
  ...
  <body>
      <div class="container">
          ...
          <div class="section">
              ...
              <p>Total Number of days worked: <span class="line">{{ $attendedDays }}</span> </p>
              ...
              <p></p>
              <p><i>Signature:</i> <img class="line-img" src="{{ public_path('storage/' . $employer->supervisorSignature) }}" alt="Signature" width="100"> <i>Official Stamp:</i> <img class="line-img" src="{{ public_path('storage/' . $employer->Muhuri) }}" alt="Stamp" width="100"></p>
          </div>
          ...
          @for ($week = 1; $week <= 8; $week++)
            ...
            <div class="section">
                <p>Position Section: <span class="line">{{ $employer->fieldworkTitle }}</span></p>
                <p>Date (From): ...................................... To: ...............................................................</p>
            </div>
            ...
            <div class="section">
                ...
                @php
                    $daysAttended = 0;
                    $allDaysPresent = true; // Flag to check if all 5 days are present
                @endphp
                @for ($day = 1; $day <= 5; $day++)
                    @php
                    ...
                    @endphp
                    <p>Day {{ $day }}</p>
                    <p class="line">{{ $logs[$dayNumber] ?? 'No log entry' }}</p>
                    @if ($attended[$dayNumber] === 'present')
                        @php
                            $daysAttended++;
                        @endphp
                    @else
            ...
            @if ($allDaysPresent)
                <div class="section">
                    ...
                    <p>Comments (If any): <strong class="line capitalize">{{ $student->studentName }} attended all 5 days this week! Well done!</strong></p>
            ...
            @else
                <div class="section">
                    <h2>Weekly Comments by Host Supervisors</h2>
                    <p>Supervisor's Name: <span class="line capitalize">{{ $employer->supervisorName }}</span></p>
                    <p>Comments (If any): <strong class="line capitalize">{{ $student->studentName }} did not attend all 5 days this week, was absent on some of those days.</strong></p>
                    <p><i>Signature:</i> <img class="line-img" src="{{ public_path('storage/' . $employer->supervisorSignature) }}" alt="Signature" width="100"> <i>Official Stamp:</i> <img class="line-img" src="{{ public_path('storage/' . $employer->Muhuri) }}" alt="Stamp" width="100"> Date.........................</p>
                </div>
            @endif
        @endfor
   ...
   <head>
       ...
       <style>
           ...
           .spacer {
            margin-bottom: 100px; /* Adjust this value based on your requirements */
        }
   ...
   <body>
       ...
       <div class="container cover-page">
           ...
           <h2>STUDENT LOG BOOK</h2>
           <h2>FOR FIELD PRACTICAL TRAINING</h2>
           <h2 class="uppercase">{{ $employer->companyName }}</h2>
           <h2 class="uppercase">{{ $employer->location }}</h2>
           @php
               use Carbon\Carbon;
               $currentYear = Carbon::now()->year;
           @endphp
           ...
           <div class="section">
               <p class="center-content bold">Name of the Student: <span class="uppercase">{{ $student->studentName }}</span></p>
               <p class="center-content bold">Reg. No: <span class="uppercase">{{ $student->registrationID }}</span></p>
               <p class="center-content bold">Programme of Study: <span>{{ $student->course }}</span></p>
               <p class="spacer center-content bold">Year: <span class="line">{{ $student->studyYear }}</span></p>
           </div>
        </div>
           <div class="section">
               <h4>[ To be filled by IFM Staff ]</h4>
           ...
           <p class="note">Every day student will make a summarized entry of the work done and lessons learnt. These entries will be checked by host supervisor for accuracy and relevancy and confirmed by Institute field report writing supervisor.</p>
           <p class="warning spacer">Entries not signed will not be accepted for assessment.</p>
